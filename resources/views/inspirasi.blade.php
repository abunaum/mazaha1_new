...
@section('scripts')
    ...
    <script>
        ...
        function cariSpesifik(page, type, nama) {
            ...
            $('#loading').show();
            const url = '{{ route("api-inspirasi-public") }}?page=' + page + '&type=' + type + '&nama=' + nama;
            $.ajax({
                url: url,
                type: 'GET',
                dataType: 'json',
        ...
        function searchBerita(page, keyword) {
            $('.berita > *:not(#loading)').hide();
            $('#loading').show();
            const url = '{{ route("api-inspirasi-public") }}?page=' + page + '&search=' + keyword;
            $.ajax({
                url: url,
                type: 'GET',
                dataType: 'json',
        ...
        function getData(page) {
            $('.berita > *:not(#loading)').hide();
            $('#loading').show();
            const url = '{{ route("api-inspirasi-public") }}?page=' + page;
            $.ajax({
                url: url,
                type: 'GET',
                dataType: 'json',
        ...
        function loopdata(data) {
            ...
            if (data.length >= 1) {
                ...
                $.each(data, function (index, value) {
                        ...
                        if (id >= 2) {
                            const urlberitanya = "{{ route('inspirasi-detail', '') }}" + '/' + slug;
                            const card = `
  ...
  `;
                            ...
                            ;
                            const urlslug = "{{ route('inspirasi-detail', '') }}" + '/' + slug;
